...
2. Choose a **Match Mode** appropriate for the material.
...
It intentionally does not contain an installer, developer tools, tests, or
debug symbols. Per-file hashes are in `docs\BUILD_MANIFEST.txt`.
...
Tone Trace EQ 1.0 ships as **CLAP**. The CLAP implementation is the tested reference release and contains the complete Tone Trace workflow and matching engine.
...
## 3. Choosing useful material

The quality of the match depends more on the captures than on chasing a particular confidence number.

For microphone or signal-chain matching, use comparable performances whenever possible. The recordings do not need sample-accurate time alignment, but they should contain enough similar spectral information for the comparison to mean something.

For voice, include a representative spread of vowels, consonants, pitch, and level. For music, use material that represents the source you actually intend to process. A short, unusual passage can produce a technically valid but unhelpful match.
...
| --- | --- |
| **Full Mix** | General music and mixed program material. |
...
The modes change how repeated spectral energy, smoothing, and stability are evaluated. They are not quality levels.
...
Negative strength is useful for checking the inverse relationship or for special effects, but normal matching uses positive values.
...
Each band is drawn visually as a vertical graphic-EQ fader, with its frequency above it, a clear 0 dB center mark, and the exact final dB value below the fader. Sighted users can click or drag a fader, use the mouse wheel for 1 dB steps, or double-click to center that band at 0 dB. Hover, focus, positive gain, and negative gain have distinct visual feedback. The same control remains a native readonly edit for accessibility, so NVDA keeps the concise band/frequency/value announcement and the existing tab order rather than gaining extra slider controls or verbosity.
...
The command-line exporter can render direct IRs at:
...
A Tone Trace IR can be loaded into a convolution host such as ReaVerb. The IR represents the linear correction only, so downstream headroom management remains your responsibility.
...
### The match is technically valid but sounds strange

Use more representative capture material, especially for speech or highly variable music. Also try reducing Correction Strength or Maximum Correction before assuming the model itself is wrong. If Full Correction Range is enabled, turn it off while troubleshooting so Maximum Correction limits the size of the match.

For Voice mode, Tone Trace automatically backs away from unusually dense narrow detail when its correction would otherwise become excessively ringy or phasey-sounding. If two different speech performances still produce a match you do not like, try another post-capture Match Mode before recapturing; the retained pair can be reinterpreted immediately.
...
The release package also includes tools for offline work and verification.
...
| Match Mode | 0–4 | Full Mix | writable |
...
Tone Trace EQ 1.0.0 is the first stable public source release. The matching engine, state handling, CLAP workflow, full-capture fallback, and balanced band-page layout are covered by automated tests. As with any audio plug-in release, the exact Windows binary should still receive a final host and accessibility check before it is attached to a public release.
